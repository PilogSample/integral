		...
		success: function(response) {
			if (response != null && !jQuery.isEmptyObject(response)) {
				if (response['selectFlag']) {
					...
					var dataTypeCountObj = JSON.stringify(dataTypeCountObj);
					dataTypeCountObj = dataTypeCountObj.replaceAll('"', '#');
					showQueryCharts(resultQuery,columnsList, tableName, joinQueryFlag,"Y",dataTypeCountObj,methodName);
	...
	$.ajax({
		...
		success: function(response) {
			...
			if (response != null && !jQuery.isEmptyObject(response)) {
				...
				var insightList = response['insightList'];
				...
				if (insightList != null && !jQuery.isEmptyObject(insightList) &&
					...
					getInsightsView(tableName,insightList, querysMap);
                 } else {
                    var modalObj = {
						title: 'Message',
...
function getInsightsView(tableName,insightsData,queryMap)
{
	insightsQueryMapArr[tableName]= queryMap;
	var ulStr = "<ul> <h6>Insights</h6>";
	$.each(insightsData,function(i,val)
	{
		ulStr += "<li onclick='showInsightsChartAndData(\""+val+"\",\""+tableName+"\")'>"+val+"</li>";
	});
	ulStr += "</ul>";
	$("#visionChartsAutoSuggestionUserId1").html(ulStr);
	switchSmartBiDesignTabs('li_autoSuggestionsView', 'visionChartAutoSuggestionsViewId');
